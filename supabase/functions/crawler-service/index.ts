...
class RobustCrawler {
  ...
  async processJob(job: CrawlJob): Promise<void> {
    console.log(`Processing job ${job.id} for website ${job.website_id}`)
    ...
    try {
      ...
      const results = await this.processUrlsBatch(urls, job.metadata.settings, job.id, scan.id)
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'process_job' && jobId) {
        ...
        const { data: job, error } = await crawler.supabase
          .from('jobs')
          .select('*')
          .eq('id', jobId)
          .single()
        ...
        // Process the job
        await crawler.processJob(job)
        
        return new Response(
          JSON.stringify({ success: true, message: 'Job processed successfully' }),
